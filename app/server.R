...
shinyServer(function(input, output) {
  ...
  output$general_boxplot <-renderPlotly({
    graph_y_name <- list(title = paste('The number of', input$overview_box))
  ...
  # overview:barplot, reactive
   ...
   output$barplot <- renderGvis({
      gvisBarChart(data = barplot_data(),
                   options = list(chartArea="{left:170,top:0, right:150}",
                                           height = 800,
                                           width = 'automatic',
                                           bar="{groupWidth:'80%'}"
                                           ))
  ...
  # attribute: corrplot, reactive
   ...
   output$scatterplot <- renderPlotly({
       ...
       graph_x_name <- list(title = paste("The number of", input$x_axis))
       graph_y_name <- list(title = paste("The number of", input$y_axis))
   ...
   # wordcloud: title, reactive
     ...
     output$lk_in = renderMenu ({
       menuItem("LinkedIn", icon = icon("linkedin-square"),
                href = "https://www.linkedin.com/in/zhiruizhang/")
     })
     
     # link to my blog
     output$blg = renderMenu ({
       menuItem("Blog", icon = icon("link"),
                href = "https://docs.google.com/document/d/1VezXKL2g7uzNY58cMfgcuBxzeaJhX66R_MXovdHsahU/edit?usp=sharing")
     })
